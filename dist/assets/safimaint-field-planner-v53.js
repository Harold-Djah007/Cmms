...
(function(){
  ...
  async function submitPortalRequest(){
    const assetId=document.getElementById('v53RequestAsset')?.value,summary=document.getElementById('v53RequestSummary')?.value.trim(),description=document.getElementById('v53RequestDescription')?.value.trim(),requester=document.getElementById('v53Requester')?.value.trim(),urgency=document.getElementById('v53RequestUrgency')?.value||'Normal',files=[...(document.getElementById('v53RequestFiles')?.files||[])];
    if(!assetId||!summary||!requester){toast('Asset, issue summary and requester are required');return}
    const r={id:uid('WR'),assetId,summary,description,urgency,requester,status:'Requested',createdAt:iso(),requestedAt:iso(),reviewerId:null,reviewedBy:null,triageNote:'',workOrderId:null,history:[{at:iso(),text:'Request submitted by '+requester}]};state.requests.unshift(r);addAudit('REQUEST_CREATED',r.id,summary);dispatchEvent('Maintenance request created',r.id+' submitted',summary,{assetId,relatedId:r.id});saveState();
    if(files.length&&window.SafiFiles){for(const file of files){try{await SafiFiles.add('request',r.id,file)}catch(err){toast(file.name+': '+err.message)}}r.history.unshift({at:iso(),text:files.length+' attachment'+(files.length===1?'':'s')+' captured with request'});saveState();setTimeout(()=>window.SafiAttachmentSync?.syncPending({quiet:true}),150)}
    toast(r.id+' submitted');go('requests');
  }
  ...
  function printWork(id){
    ...
    const laborCost=labor.reduce((n,x)=>n+Number(x.hours||0)*Number(getUser(x.userId)?.hourlyRate||0),0),partCost=parts.reduce((n,x)=>n+(x.actualCost!==undefined?Number(x.actualCost||0):Number(x.actual||0)*Number(getPart(x.partId)?.lastPrice||getPart(x.partId)?.unitCost||0)),0),misc=(w.miscCosts||[]).reduce((n,x)=>n+Number(x.amount||0),0);
