...
(function(){
  ...
  const genericAssetList=(title,subtitle,filter)=>()=>{
    ...
    return '<div class="v50-page">'+pageHead('Asset management',title,subtitle,'<button class="button" data-route="assets">Hierarchy</button><button class="button primary" data-action="add-asset">＋ Add asset</button>')+
      '<div class="v50-toolbar"><input data-v51-asset-search placeholder="Search code, name, location…" value="'+esc(ui.v51AssetSearch||'')+'"><span class="grow"></span><span class="v50-pill" data-v51-asset-count>'+rows.length+' records</span></div>'+
      ...
      rows.map(a=>{const p=getAsset(a.parentId),ow=state.workOrders.filter(w=>(w.assetIds||[]).includes(a.id)&&control(w)!=='CLOSED').length,pm=state.scheduledMaintenance.filter(x=>(x.assetIds||[x.assetId]).includes(a.id)&&!x.paused).sort((x,y)=>String(x.nextDue||'').localeCompare(String(y.nextDue||'')))[0],blob=[a.code,a.name,a.location,a.category,a.type,p?.name].filter(Boolean).join(' ').toLowerCase();return '<tr class="clickable" data-v51-asset-row data-v51-search="'+esc(blob)+'" data-v51-open-asset="'+esc(a.id)+'" '+(ui.v51AssetSearch&&!blob.includes(ui.v51AssetSearch.toLowerCase())?'hidden':'')+'><td><strong>'+esc(a.code)+' · '+esc(a.name)+'</strong><small>'+esc(a.type)+'</small></td><td>'+esc(a.category||'—')+'</td><td>'+esc(p?.name||'Top level')+'</td><td>'+esc(a.location||getAsset(a.locationId)?.name||'—')+'</td><td>'+esc(a.criticality||'—')+'</td><td>'+ow+'</td><td>'+esc(pm?.nextDue||'—')+'</td><td><span class="v51-asset-status '+(a.operatingState==='Offline'?'offline':'')+'"><i></i>'+esc(a.operatingState||'Unknown')+'</span></td></tr>'}).join('')+
      '</tbody></table></div></div>';
  };
  ...
  document.addEventListener('input',e=>{
    if(!e.target.matches('[data-v51-asset-search]'))return;
    ui.v51AssetSearch=e.target.value;
    const q=ui.v51AssetSearch.trim().toLowerCase(),rows=[...document.querySelectorAll('[data-v51-asset-row]')];
    let shown=0;
    rows.forEach(row=>{const hit=!q||String(row.dataset.v51Search||'').includes(q);row.hidden=!hit;if(hit)shown++});
    const count=document.querySelector('[data-v51-asset-count]');if(count)count.textContent=shown+' record'+(shown===1?'':'s');
  },true);
