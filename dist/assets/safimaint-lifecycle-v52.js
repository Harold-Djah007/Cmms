...
(function(){
  ...
  function renderRequestsV52(){
    ...
    const filter=ui.v52RequestStatus||'All',q=String(ui.v52RequestSearch||'').trim().toLowerCase();
    const rows=state.requests.filter(r=>filter==='All'||r.status===filter).sort((a,b)=>String(b.createdAt).localeCompare(String(a.createdAt)));
    const shown=rows.filter(r=>!q||[r.id,r.summary,r.description,r.requester,getAsset(r.assetId)?.name,getAsset(r.assetId)?.code].join(' ').toLowerCase().includes(q));
    return '<div class="v50-page">'+pageHead('Maintenance','Work requests','Low-friction maintenance reporting with controlled planner triage before resources are committed.','<button class="button primary" data-action="add-request">＋ New request</button>')+
      ...
      '<div class="v50-toolbar"><input data-v52-request-search value="'+esc(ui.v52RequestSearch||'')+'" placeholder="Search requests, assets, requester…"><select data-v52-request-status>'+statuses.map(s=>'<option '+(filter===s?'selected':'')+'>'+s+'</option>').join('')+'</select><span class="grow"></span><span class="v50-pill" data-v52-request-count>'+shown.length+' record'+(shown.length===1?'':'s')+'</span></div>'+
  ...
  document.addEventListener('input',e=>{if(e.target.matches('[data-v52-request-search]')){ui.v52RequestSearch=e.target.value;const q=ui.v52RequestSearch.trim().toLowerCase(),rows=[...document.querySelectorAll('[data-v52-request-row]')];let shown=0;rows.forEach(row=>{const hit=!q||String(row.dataset.search||'').includes(q);row.hidden=!hit;if(hit)shown++});const count=document.querySelector('[data-v52-request-count]');if(count)count.textContent=shown+' record'+(shown===1?'':'s');const empty=document.querySelector('[data-v52-request-empty]');if(empty)empty.hidden=shown>0}},true);
