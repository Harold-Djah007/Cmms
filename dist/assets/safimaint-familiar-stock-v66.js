...
(function(){
  ...
  showStockLocation=function(p,index=null){
    ...
    const l=existing||{storeId:state.stores[0]?.id||'',aisle:'',row:'',bin:'Unassigned',onHand:0,min:0,max:0,active:true};
    ...
    openModal({eyebrow:'Stock · '+p.code,title:existing?'Stock Location':'Add Stock Location',submitText:'OK',body:
      ...
      '<div class="v66-location-fields">'+
        '<label>Location<select name="storeId">'+state.stores.map(s=>'<option value="'+esc(s.id)+'" '+(s.id===l.storeId?'selected':'')+'>'+esc(s.name)+'</option>').join('')+'</select></label>'+
      ...
      onSubmit:fd=>{
        ...
        const duplicate=p.locations.find((x,i)=>i!==index&&x.storeId===storeId&&String(x.aisle||'')===aisle&&String(x.row||'')===row&&String(x.bin||'')===bin);
