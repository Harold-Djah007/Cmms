...
(function(){
  ...
  function filesRecord(a){return `<section class="fx23-section"><div class="fx23-section-head"><strong>Files</strong><button class="button" type="button" data-local-file-pick="${esc(a.id)}">＋ Add files</button><input class="file-input-hidden" type="file" multiple data-local-file-input="${esc(a.id)}" accept="application/pdf,image/jpeg,image/png,image/webp,text/plain,text/csv,.doc,.docx,.xls,.xlsx"></div><div class="attachment-list" data-local-file-list="${esc(a.id)}"><div class="fx23-inline-empty">No manuals, photos or attachments have been added yet.</div></div></section>`}
