...
(function(){
  ...
  function bomTab(a){
    const parts=bom(a),allWork=linkedWork(a),openWork=allWork.filter(w=>control(w)!=='CLOSED'),workIds=new Set(allWork.map(w=>w.id)),usage=(state.stockTransactions||[]).filter(t=>t.workOrderId&&workIds.has(t.workOrderId)&&Number(t.qty)<0);
    const low=parts.filter(p=>partOnHand(p)<Number(p.min||0));
    const stockValue=parts.reduce((n,p)=>n+partOnHand(p)*Number(p.lastPrice||p.unitCost||0),0);
    const locationText=p=>(p.locations||[]).filter(l=>l.active!==false).map(l=>{const bits=[getStore(l.storeId)?.name||l.storeId,l.aisle&&'Aisle '+l.aisle,l.row&&'Row '+l.row,l.bin&&'Bin '+l.bin].filter(Boolean);return '<span class="v63-location-chip">'+esc(bits.join(' · '))+' · '+Number(l.onHand||0)+' '+esc(p.uom)+'</span>'}).join('')||'<span class="v63-location-chip muted">No stock location</span>';
    const plannedFor=p=>openWork.reduce((n,w)=>n+(w.parts||[]).filter(x=>x.partId===p.id).reduce((m,x)=>m+Math.max(0,Number(x.planned||0)-Number(x.actual||0)),0),0);
    const supplier=p=>state.businesses?.find(b=>b.id===p.preferredBusinessId)?.name||getVendor(p.vendorId)?.name||'—';
    return '<section class="v45-section"><div class="v45-section-head"><strong>Asset parts / Bill of Materials</strong><div class="v45-section-actions"><span>'+parts.length+' linked to '+esc(a.code)+'</span><button class="button small" type="button" data-route="inventory">Open full inventory</button><button class="button small primary" type="button" data-v45-add-bom="'+esc(a.id)+'">＋ Add BOM part</button></div></div>'+
      '<div class="v63-asset-stock-summary"><div><small>Linked parts</small><strong>'+parts.length+'</strong><span>Asset BOM</span></div><div class="'+(low.length?'warn':'')+'"><small>Below minimum</small><strong>'+low.length+'</strong><span>Need replenishment</span></div><div><small>Open-work demand</small><strong>'+parts.reduce((n,p)=>n+plannedFor(p),0).toFixed(1)+'</strong><span>Units still planned</span></div><div><small>Stock value</small><strong>'+money(stockValue)+'</strong><span>Linked inventory value</span></div></div>'+
      (parts.length?'<div class="v45-table-wrap"><table class="v45-table v63-bom-table"><thead><tr><th>Part</th><th>Standard / demand</th><th>Stock</th><th>Stock locations</th><th>Min / max</th><th>Supplier / cost</th><th>Actions</th></tr></thead><tbody>'+parts.map(p=>{const on=partOnHand(p),minimum=Number(p.min||0),maximum=Number(p.max||0),planned=plannedFor(p);return '<tr><td><strong>'+esc(p.code)+' · '+esc(p.name)+'</strong><small>'+esc(p.category||'Part')+' · '+esc(p.uom)+'</small></td><td><strong>'+Number(a.bomQuantities?.[p.id]||1)+' '+esc(p.uom)+'</strong><small>'+planned.toFixed(1)+' '+esc(p.uom)+' planned on open work</small></td><td><strong class="'+(on<minimum?'v63-low':'')+'">'+on+' '+esc(p.uom)+'</strong><small>'+(on<minimum?'Below minimum':'Available')+'</small></td><td><div class="v63-location-list">'+locationText(p)+'</div></td><td><strong>'+minimum+' / '+maximum+'</strong><small>Minimum / maximum</small></td><td><strong>'+esc(supplier(p))+'</strong><small>'+money(Number(p.lastPrice||p.unitCost||0))+' last/unit price</small></td><td><div class="v63-bom-actions"><button class="button small" data-v45-stock-bom="'+esc(a.id)+'|'+esc(p.id)+'">Stock</button><button class="button small primary" data-v45-issue-bom="'+esc(a.id)+'|'+esc(p.id)+'">Issue</button><button class="button small" data-v45-request-bom="'+esc(a.id)+'|'+esc(p.id)+'">Request</button><button class="button small" data-open-part="'+esc(p.id)+'">Full record</button><button class="button small danger" data-v45-remove-bom="'+esc(a.id)+'|'+esc(p.id)+'">Remove</button></div></td></tr>'}).join('')+'</tbody></table></div>':empty('No asset parts linked','Add the spare parts normally used on this asset. They remain managed globally in Stock & parts while this tab shows the parts that belong to this asset.'))+
      '</section>'+
      '<section class="v45-section"><div class="v45-section-head"><strong>Recent parts usage on this asset</strong><span>'+usage.length+' issue transaction'+(usage.length===1?'':'s')+'</span></div>'+
      (usage.length?'<div class="v45-table-wrap"><table class="v45-table"><thead><tr><th>Date</th><th>Part</th><th>Quantity</th><th>Work order</th><th>Location</th><th>Reference</th></tr></thead><tbody>'+usage.slice(0,20).map(t=>{const p=getPart(t.partId),loc=[getStore(t.storeId)?.name||t.storeId,t.aisle&&'Aisle '+t.aisle,t.row&&'Row '+t.row,t.bin&&'Bin '+t.bin].filter(Boolean).join(' · ');return '<tr><td>'+dateTimeFmt(t.at)+'</td><td><strong>'+esc(p?.code||t.partId)+'</strong><small>'+esc(p?.name||'')+'</small></td><td>'+Math.abs(Number(t.qty))+' '+esc(p?.uom||'')+'</td><td><button class="button small" data-open-work="'+esc(t.workOrderId)+'">'+esc(t.workOrderId)+'</button></td><td>'+esc(loc||'—')+'</td><td>'+esc(t.reference||'—')+'</td></tr>'}).join('')+'</tbody></table></div>':empty('No parts consumed yet','Issue a linked part to an open work order on this asset and the transaction will appear here automatically.'))+
      '</section>';
  }
  ...
  document.addEventListener('click',e=>{
    ...
    const stockBom=e.target.closest('[data-v45-stock-bom]');if(stockBom){e.preventDefault();e.stopImmediatePropagation();const [aid,pid]=stockBom.dataset.v45StockBom.split('|'),a=getAsset(aid),p=getPart(pid);if(!a||!p)return;const locs=p.locations||[];if(!locs.length){if(typeof showStockLocation==='function')showStockLocation(p,null);return}openModal({eyebrow:a.code+' · '+p.code,title:'Stock locations for '+p.name,body:'<div class="v63-stock-location-modal">'+locs.map((l,i)=>'<button type="button" class="v63-stock-location-card" data-v62-stock-location="'+esc(p.id)+'|'+i+'"><span><strong>'+esc(getStore(l.storeId)?.name||l.storeId||'Store')+'</strong><small>'+esc([l.aisle&&'Aisle '+l.aisle,l.row&&'Row '+l.row,l.bin&&'Bin '+l.bin].filter(Boolean).join(' · ')||'No aisle / row / bin detail')+'</small></span><b>'+Number(l.onHand||0)+' '+esc(p.uom)+'</b><em>'+((l.active===false)?'Inactive':'Open settings')+' ›</em></button>').join('')+'</div><div class="v63-stock-modal-footer"><button class="button primary" type="button" data-v62-add-location="'+esc(p.id)+'">＋ Add stock location</button></div>'});return}
    const issueBom=e.target.closest('[data-v45-issue-bom]');if(issueBom){e.preventDefault();e.stopImmediatePropagation();const [aid,pid]=issueBom.dataset.v45IssueBom.split('|'),a=getAsset(aid),p=getPart(pid);if(!a||!p)return;const work=linkedWork(a).filter(w=>control(w)!=='CLOSED'),locs=(p.locations||[]).filter(l=>l.active!==false&&Number(l.onHand||0)>0);if(!work.length){toast('Create or open a work order on this asset before issuing parts');return}if(!locs.length){toast('No active stock location has quantity available for '+p.code);return}openModal({eyebrow:a.code+' · '+p.code,title:'Issue part to work order',submitText:'Issue part',body:'<div class="form-grid">'+field('workOrderId','Work order',work[0].id,{type:'select',options:work.map(w=>({value:w.id,label:w.id+' · '+w.title}))})+field('location','Stock location',locs[0].storeId+'|'+locs[0].bin,{type:'select',options:locs.map(l=>({value:l.storeId+'|'+l.bin,label:(getStore(l.storeId)?.name||l.storeId)+' · '+[l.aisle&&'Aisle '+l.aisle,l.row&&'Row '+l.row,l.bin&&'Bin '+l.bin].filter(Boolean).join(' · ')+' · '+Number(l.onHand||0)+' '+p.uom+' available'}))})+field('qty','Quantity to issue',String(Number(a.bomQuantities?.[p.id]||1)),{type:'number',min:.01,step:'.01'})+field('reference','Reference / note',a.code+' BOM',{span:true})+'</div>',onSubmit:fd=>{try{const [storeId,bin]=String(fd.get('location')).split('|'),wid=String(fd.get('workOrderId')),qty=Number(fd.get('qty'));if(!Number.isFinite(qty)||qty<=0){toast('Enter a valid quantity');return}postStock(p.id,'Issue',qty,storeId,bin,{workOrderId:wid,reference:String(fd.get('reference')||a.code+' BOM'),note:'Issued from '+a.code+' asset parts tab'});state.assetEvents.unshift({id:uid('AE'),assetId:a.id,type:'Part issued',at:iso(),userId:CURRENT_USER,detail:p.code+' × '+qty+' issued to '+wid});saveState();closeModal();refresh();toast(qty+' '+p.uom+' '+p.code+' issued to '+wid)}catch(err){toast(err.message)}}});return}
    const requestBom=e.target.closest('[data-v45-request-bom]');if(requestBom){e.preventDefault();e.stopImmediatePropagation();const [aid,pid]=requestBom.dataset.v45RequestBom.split('|'),a=getAsset(aid),p=getPart(pid);if(!a||!p)return;const existing=state.purchaseRequests.find(r=>r.partId===p.id&&!['Received','Closed','Cancelled','Rejected'].includes(r.status));if(existing){toast(existing.id+' already covers '+p.code);return}const onHand=partOnHand(p),target=Math.max(Number(p.max||0),Number(p.min||0)+Number(a.bomQuantities?.[p.id]||1)),qty=Math.max(1,Number(p.reorderQty||0),target-onHand);const pr={id:uid('PR'),partId:p.id,qty,quantity:qty,requestedQty:qty,status:'Requested',createdAt:iso(),reason:'Asset '+a.code+' BOM replenishment · on hand '+onHand+' '+p.uom,source:'Asset BOM',assetId:a.id};state.purchaseRequests.unshift(pr);addAudit('PURCHASE_REQUEST_CREATED',pr.id,a.code+' · '+p.code+' × '+qty);dispatchEvent('Purchase request created',pr.id+' created',a.code+' requires '+p.name+' replenishment',{assetId:a.id,partId:p.id,relatedId:pr.id});saveState();refresh();toast(pr.id+' submitted for '+p.code);return}
