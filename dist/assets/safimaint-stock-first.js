...
(function(){
  ...
  function stockDashboard(){
    ...
    return '<div class="sf-dashboard">'+
      '<section class="sf-command-hero"><div class="sf-command-main"><div class="sf-command-copy"><div class="sf-live-label"><i></i> Operations inventory · live</div><h1>Stockroom overview</h1><p>Good morning, '+esc(user?.name?.split(' ')[0]||'Harold')+'. Count stock, post movements and see what maintenance needs from one workspace.</p><div class="sf-hero-actions"><button class="button primary" data-action="cycle-count">'+sfIcon('count')+'Start cycle count</button><button class="button" data-action="global-stock-move">'+sfIcon('move')+'Post movement</button><button class="button" data-action="add-part">'+sfIcon('part')+'Add stock item</button></div></div>'+heroVisual()+'</div><div class="sf-shift-strip"><div><span>Stock readiness</span><strong>'+readiness+'%</strong></div><div><span>Active stores</span><strong>'+stores.length+'</strong></div><div><span>Locations tracked</span><strong>'+locations+'</strong></div><div><span>Inventory value</span><strong>'+money(totalValue)+'</strong></div></div></section>'+
      '<section class="sf-kpis">'+kpi('Stock items',parts.length,locations+' active locations','inventory','stock','',100)+kpi('Below minimum',low.length,critical.length+' stocked out','planning','alert',low.length?'sf-kpi-alert':'',parts.length?100-low.length/parts.length*100:100)+kpi('Count coverage',coverage+'%',countedThisMonth+' counted this month','counts','count','',coverage)+kpi('Open maintenance',openWork.length,offline.length+' asset'+(offline.length===1?'':'s')+' offline','work-orders','work',offline.length||openWork.some(w=>w.priority==='Critical')?'sf-kpi-bad':'',Math.min(100,openWork.length*12))+'</section>'+
      ...
      '<article class="sf-panel"><div class="sf-panel-head"><div><h2>7-day movement activity</h2><p>Receipts, issues, transfers and count adjustments</p></div><div class="sf-live-label"><i></i> Live</div></div><div class="sf-activity-chart">'+bars.map(b=>'<div class="sf-chart-col" title="'+b.value+' movement'+(b.value===1?'':'s')+'"><b>'+b.value+'</b><i style="height:'+b.height+'%"></i><small>'+b.label+'</small></div>').join('')+'</div><div class="sf-summary-list"><div class="sf-summary-row"><span>Moves this week</span><b>'+bars.reduce((n,b)=>n+b.value,0)+'</b></div><div class="sf-summary-row"><span>Counted this month</span><b>'+countedThisMonth+' of '+parts.length+'</b></div><div class="sf-summary-row"><span>Stock readiness</span><b>'+readiness+'%</b></div></div></article></section>'+
